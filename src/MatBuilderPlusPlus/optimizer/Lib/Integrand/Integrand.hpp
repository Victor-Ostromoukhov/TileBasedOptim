...
// Function to estimate the integral of a function by evaluating it in certain points
...
// Function to estimate the integral by moving only one point
template <typename VECTYPE>
...
inline double multivariateGaussianIntegration(const std::vector<VECTYPE>& points, const VecXDynamic& m, const MatXDynamic& sigma,int integrandType){
    ...
    std::function<double(const VECTYPE&)> f = [&integrand,&integrandType](const VECTYPE& point){ return integrand.eval(point,integrandType); }; // Creation of f,so that f(xi) = yi
