...
public:
    ...
    explicit inline VecX(const VecXDynamic &v);
...
template<int N>
inline VecX<N>::VecX(const VecXDynamic &v) {
    for (int i = 0; i < N; ++i) {
        coefs[i] = v.operator[](i);
    }
}

//inline VecX<N>(const VecX<N><N>& b){}
...
inline VECTYPE randomUnitVector(int d, RandomGenerator &engine) {
    std::normal_distribution<double> normalDistribution(0, 1);
    VECTYPE v(d);
    for (int i = 0; i < d; ++i) {
        v[i] = normalDistribution(engine);
    }
    v.normalize();
    return v;
}
