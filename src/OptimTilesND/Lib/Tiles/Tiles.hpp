...
inline void Tiles<dimension>::vectorPointSumRand(Points<dimension>* point,Points<dimension> refPoint,double valToMultiplyBy,int indice,DirectionnalVector dV){
  if (dimension == 2) {
    ...
    if (dimension == 3) {
      ...
      point->dimensionnalArray[dV.getImpDim()-1] = (refPoint.dimensionnalArray[dV.getImpDim()-1] + dV.getImpVal()*(delta + valToMultiplyBy) / 4);
    }else{
      point->dimensionnalArray[dV.getImpDim()-1] = (refPoint.dimensionnalArray[dV.getImpDim()-1]+dV.getImpVal()*valToMultiplyBy);
    }
  }
}
